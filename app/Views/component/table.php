        ...
        <tbody>
            ...
            foreach ($datatable as $dt) : $no++; ?>
                <tr>
                    ...
                    <td><?= $dt['no_printing'] ?></td>
                    <td><?= $dt['jumlah_meter'] ?></td>
                    <td><?= $dt['keterangan'] ?></td>
                    <td>
                        <button onclick="editForm('<?= $dt['id'] ?>')" class="btn btn-warning text-white"><i class='bx bx-edit-alt'></i></button>
                        <button id="<?php echo 'updateFormButton' . $dt['id'] ?>" onclick="setOnSubmit('updatePrinting(<?= $dt['id'] ?>, event)')" type="button" data-bs-toggle="modal" data-bs-target="#tambahModal" data-bs-judul="Update Data" class="d-none" data-bs-aksi="updatePrinting('<?= $dt['id'] ?>', event)"></button>
